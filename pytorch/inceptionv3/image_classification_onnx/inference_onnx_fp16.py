...
# Convert inpu tensor to FP16
if torch.cuda.is_available():
    input_batch = input_batch.to(torch.float16)

# Convert input tensor to numpy array
if torch.cuda.is_available():
    input_array = input_batch.cpu().numpy().astype(np.float16)
else:
    input_array = input_batch.numpy().astype(np.float16)

# Set Execution Provider for ONNX Runtime
session_fp16 = onnxruntime.InferenceSession("inceptionv3_model_fp16.onnx", providers=['ROCMExecutionProvider'])

# warm up run to load model and data onto gpu first
if torch.cuda.is_available():
    input_batch = input_batch.cpu()
ort_outputs = session_fp16.run([], {'x.1': input_batch.numpy()})[0]
